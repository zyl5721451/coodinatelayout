...
public class Activity extends ContextThemeWrapper
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (mWindow.shouldCloseOnTouch(this, event)) {//点击外部区域Activity消失，消费事件，返回true
            finish();
            return true;
        }

        return false;//默认自身OnTouchEvent不处理事件
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            onUserInteraction();//用来实现屏保通知删除等功能，触发activtiy在栈顶时，删除通知事件等。
        }
        if (getWindow().superDispatchTouchEvent(ev)) {//Window--->PhoneWindow-->DecorView-->ViewGroup中DispatchTouchEvent()
            return true;
        }
        return onTouchEvent(ev);//自身处理onTouchEvent事件。
    }
